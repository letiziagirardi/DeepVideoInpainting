...
def get_file_list2(data_dir):
    assert os.path.exists(data_dir), 'Directory {} not found.'.format(data_dir)

    file_list = []
    file_glob = os.path.join(data_dir)
    file_list.extend(glob.glob(file_glob))
...
def tf_read_image(file_name, channels=3, img_type=tf.float32, div_val=255.0, outputsize=None, random_flip_key=None):
    image_string = tf.compat.v1.read_file(file_name)
    image_decoded = tf.compat.v1.div(tf.cast(tf.image.decode_png(image_string, channels=channels), img_type), div_val)
    if outputsize:
        image_decoded = tf.cast(tf.compat.v1.image.resize_images(image_decoded, outputsize, align_corners=True, method=0), img_type)
    if random_flip_key is not None:
        image_decoded = tf.cond(tf.less(random_flip_key[0], .5), lambda: tf.compat.v1.image.flip_up_down(image_decoded), lambda: image_decoded)
        image_decoded = tf.cond(tf.less(random_flip_key[1], .5), lambda: tf.compat.v1.image.flip_left_right(image_decoded), lambda: image_decoded)
...
def read_frame_and_masks(data_dir, mask_dir, pattern):

    image_names = get_file_list(data_dir, pattern)
    if 'Icon\r' in image_names:
        image_names.remove('Icon\r')
    label_names = get_file_list(mask_dir, pattern)
    if 'Icon\r' in label_names:
        label_names.remove('Icon\r')
    instance_num = len(image_names)
    dataset = tf.compat.v1.data.Dataset.from_tensor_slices((image_names, label_names))
    print('Read {} instances from {}'.format(instance_num, data_dir))
    return dataset, instance_num

def reader(data_dir, mask_dir):

    image_names = get_file_list2(data_dir)

    label_names = get_file_list2(mask_dir)
    instance_num = len(image_names)
    dataset = tf.compat.v1.data.Dataset.from_tensor_slices((image_names, label_names))
    print('Read {} instances from {}'.format(instance_num, data_dir))
    return dataset, instance_num
...
def read_dataset_withmsk2(data_dir, pattern, shuffle_seed=None, subset=None):
    ...
    #    image_names = image_names[:subset]
    #instance_num = len(image_names)
    #dataset = tf.compat.v1.data.Dataset.from_tensor_slices((image_names))
...
def read_dataset_with2msk(data_dir, pattern, msk_replace, shuffle_seed=None, subset=None):
    ...
    dataset = tf.compat.v1.data.Dataset.from_tensor_slices((image_names, label1_names, label2_names))
...
def read_image_withmsk(image_name, label_name, outputsize=None, random_flip=False):
    random_flip_key = tf.compat.v1.random_uniform([3,], 0, 1.0) if random_flip else None
...
def read_image_withmsk2(image_name, outputsize=None, random_flip=False):
    random_flip_key = tf.compat.v1.random_uniform([3,], 0, 1.0) if random_flip else None
    image_decoded = tf_read_image(image_name, outputsize=outputsize, random_flip_key=random_flip_key)
    return image_decoded, image_name

def read_image_with2msk(image_name, outputsize=None, random_flip=False):#(image_name, label1_name, label2_name, outputsize=None, random_flip=False):
    #random_flip_key = tf.random_uniform([3,], 0, 1.0) if random_flip else None
    #image_decoded = tf_read_image(image_name, outputsize=outputsize, random_flip_key=random_flip_key)
    ...
    random_flip_key = tf.compat.v1.random_uniform([3,], 0, 1.0) if random_flip else None
    image_decoded = tf_read_image(image_name, outputsize=outputsize, random_flip_key=random_flip_key)
    return image_decoded, image_name# label_decoded, image_name