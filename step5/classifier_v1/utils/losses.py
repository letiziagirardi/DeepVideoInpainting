...
if int(tf_ver[0])<=1 and int(tf_ver[1])<=4:
    softmax_cross_entropy_with_logits = tf.compat.v1.nn.softmax_cross_entropy_with_logits
else:
    softmax_cross_entropy_with_logits = lambda labels=None, logits=None: tf.compat.v1.nn.softmax_cross_entropy_with_logits_v2(labels=tf.stop_gradient(labels), logits=logits)
